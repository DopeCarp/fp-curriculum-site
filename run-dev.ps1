...
foreach ($listener in $listeners) {
  ...
  if (
    ...
    $process.CommandLine -like "*next*dev*" -and
    $process.CommandLine -like "*2026-04-20-you-are-helping-me-build-a*"
  ) {
    Write-Host "Stopping stale local dev server on port $port (PID $($listener.OwningProcess))..."
...
& $nodePath "node_modules\next\dist\bin\next" dev --webpack --hostname 127.0.0.1 --port $port
